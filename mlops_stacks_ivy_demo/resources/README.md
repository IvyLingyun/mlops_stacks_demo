...
The databricks CLI bundles top level is defined by file `mlops_stacks_ivy_demo/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`mlops_stacks_ivy_demo/resources/model-workflow-resource.yml`)
 - batch inference workflow (`mlops_stacks_ivy_demo/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`mlops_stacks_ivy_demo/resources/monitoring-resource.yml`)
 - feature engineering workflow (`mlops_stacks_ivy_demo/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`mlops_stacks_ivy_demo/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `mlops_stacks_ivy_demo/databricks.yml` and `mlops_stacks_ivy_demo/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-mlops-stacks-ivy-demo-model     | /dev-mlops-stacks-ivy-demo-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-mlops-stacks-ivy-demo-model | /staging-mlops-stacks-ivy-demo-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-mlops-stacks-ivy-demo-model    | /prod-mlops-stacks-ivy-demo-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-mlops-stacks-ivy-demo-model    | /test-mlops-stacks-ivy-demo-experiment    |
...
2. Complete the `TODO` in `mlops_stacks_ivy_demo/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `mlops_stacks_ivy_demo/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `mlops_stacks_ivy_demo/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-mlops-stacks-ivy-demo-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-mlops-stacks-ivy-demo-batch-inference-job`
that runs the notebook under `mlops_stacks_ivy_demo/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-mlops-stacks-ivy-demo-batch-inference-job`.
...
`test-mlops-stacks-ivy-demo-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-mlops-stacks-ivy-demo-batch-inference-job`.
